...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override = false)
	{
		...
		$query = $this->db->select('user_password, salt')
			->where('id', $id)
			->limit(1)
			->order_by('id', 'desc')
			->get(TBL_USERINFO);
		...
		if ($use_sha1_override === false && $this->hash_method == 'bcrypt') {
			...
			if ($this->bcrypt->verify($password, $hash_password_db->user_password)) {
				return true;
			}
			return false;
		}
		...
		if ($this->store_salt) {
			...
			$salt = substr($hash_password_db->user_password, 0, $this->salt_length);
			$db_password = $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
		}

		if ($db_password == $hash_password_db->user_password) {
			return true;
		} else {
			return false;
		}
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== false) {
			...
			$this->trigger_events('extra_where');
			$this->db->update(TBL_USERINFO, $data, array('id' => $id));
		} else {
			$data = array(
				'activation_code' => null,
				'active' => 1
	...
	public function clear_forgotten_password_code($code)
	{
		...
		if ($this->db->count_all_results(TBL_USERINFO) > 0) {
			$data = array(
				'forgotten_password_code' => null,
				'forgotten_password_time' => null
			);

			$this->db->update(TBL_USERINFO, $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new)
	{
		...
		$query = $this->db->select('id, user_password, salt')
		...
		$this->trigger_events('extra_where');
		$this->db->update(TBL_USERINFO, $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
			...
			->order_by("id", "ASC")
			->limit(1)
			->count_all_results(TBL_USERINFO) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update(TBL_USERINFO, $update, array($this->identity_column => $identity));
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where(ACC_GROUPS, array('name' => $this->config->item('default_group', 'ion_auth')), 1)->row();
		...
		$data = array(
			...
			'created_on' => time(),
			'user_blocked' => ($manual_activation === false ? 1 : 0)
		);
		...
		$user_data = array_merge($this->_filter_data(TBL_USERINFO, $additional_data), $data);
	...
	public function login($identity, $password, $remember = false)
	{
		...
		if (empty($identity) || empty($password)) {
			$this->session->set_flashdata('error_msg', 'Credentials Not Provided..');
		...
		if ($this->identity_column !== 'user_name') {
			$this->db->select('*')->where('user_name', $identity);
		} else {
			$this->db->select('*')
				->where('isdelete', '0')
		...
		$query = $this->db->limit(1)->order_by('id', 'desc')->get(TBL_USERINFO);
		...
		if ($this->is_time_locked_out($identity)) {
			...
			$this->session->set_flashdata('error_msg', 'Login Time Locked Out');
		...
		if ($query->num_rows() === 1) {
			...
			if ($password === true) {
				if ($user->user_blocked == 0) {
					$this->session->set_flashdata('error_msg', 'User Account is Blocked');
					$this->trigger_events('post_login_unsuccessful');
	...
	public function get_attempts_num($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$qres = $this->db->get(TBL_LOGIN_ATTEMPTS);
	...
	public function get_last_attempt_time($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$qres = $this->db->get(TBL_LOGIN_ATTEMPTS, 1);
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->db->insert(TBL_LOGIN_ATTEMPTS, array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return false;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			return $this->db->delete(TBL_LOGIN_ATTEMPTS);
		}
		return false;
	}
	...
	public function users($groups = null)
	{
		...
		if (isset($this->_ion_select) && !empty($this->_ion_select)) {
			...
			$this->db->select(array(
				TBL_USERINFO . '.*',
				TBL_USERINFO . '.id as id',
				TBL_USERINFO . '.id as user_id'
			));
		}
		...
		if (isset($groups)) {
			...
			if (isset($groups) && !empty($groups)) {
				...
				$this->db->join(
					ACC_USERS_GROUPS,
					ACC_USERS_GROUPS . '.' . $this->join['users'] . '=' . TBL_USERINFO . '.id',
					'inner'
				);
			}
			...
			if (!empty($group_names)) {
				$this->db->join(ACC_GROUPS, ACC_USERS_GROUPS . '.' . $this->join['groups'] . ' = ' . ACC_GROUPS . '.id', 'inner');
				$this->db->where_in(ACC_GROUPS . '.name', $group_names);
			}
			if (!empty($group_ids)) {
				$this->db->{$or_where_in}(ACC_USERS_GROUPS . '.' . $this->join['groups'], $group_ids);
			}
		}
		...
		$this->response = $this->db->get(TBL_USERINFO);
	...
	public function user($id = null)
	{
		...
		$this->limit(1);
		$this->order_by(TBL_USERINFO . '.id', 'desc');
		$this->where(TBL_USERINFO . '.id', $id);
	...
	public function get_users_groups($id = false)
	{
		...
		return $this->db->select(ACC_USERS_GROUPS . '.' . $this->join['groups'] . ' as id, ' . ACC_GROUPS . '.name, ' . ACC_GROUPS . '.description')
			->where(ACC_USERS_GROUPS . '.' . $this->join['users'], $id)
			->join(ACC_GROUPS, ACC_USERS_GROUPS . '.' . $this->join['groups'] . '=' . ACC_GROUPS . '.id')
			->get(ACC_USERS_GROUPS);
	}
	...
	public function add_to_group($group_ids, $user_id = false)
	{
		...
		foreach ($group_ids as $group_id) {
			if ($this->db->insert(ACC_USERS_GROUPS, array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id))) {
	...
	public function remove_from_group($group_ids = false, $user_id = false)
	{
		...
		if (!empty($group_ids)) {
			...
			foreach ($group_ids as $group_id) {
				$this->db->delete(ACC_USERS_GROUPS, array($this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id));
		...
		// otherwise remove user from all groups
			...
			if ($return = $this->db->delete(ACC_USERS_GROUPS, array($this->join['users'] => (float)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->db->get(ACC_GROUPS);
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data(TBL_USERINFO, $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update(TBL_USERINFO, $data, array('id' => $user->id));
	...
	public function delete_user($id)
	{
		...
		$this->db->delete(TBL_USERINFO, array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update(TBL_USERINFO, array('last_login' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
			...
			'user_role' => $this->Queries->read_role_info($user->user_type)->role_details,
			'user_department' => $this->Queries->read_role_info($user->user_type)->department_id,
		...
		$this->Queries->updateRecord(TBL_USERINFO, $form_data, $user->id);
	...
	public function remember_user($id)
	{
		...
		$this->db->update(TBL_USERINFO, array('remember_code' => $salt), array('id' => $id));
	...
	public function create_group($group_name = false, $group_description = '', $additional_data = array())
	{
		...
		$this->db->insert(ACC_GROUPS, $data);
	...
	public function update_group($group_id = false, $group_name = false, $additional_data = array())
	{
		...
		if (!empty($group_name)) {
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where(ACC_GROUPS, array('name' => $group_name))->row();
		...
		$group = $this->db->get_where(ACC_GROUPS, array('id' => $group_id))->row();
		...
		// filter out any data passed that doesnt have a matching column in the groups table
		// and merge the set group data and the additional data
		if (!empty($additional_data)) $data = array_merge($this->_filter_data(ACC_GROUPS, $additional_data), $data);


		$this->db->update(ACC_GROUPS, $data, array('id' => $group_id));
	...
	public function delete_group($group_id = false)
	{
		...
		$this->db->delete(ACC_USERS_GROUPS, array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->db->delete(ACC_GROUPS, array('id' => $group_id));
		// remove the group permissions
		$this->db->delete(ACC_PERMISSION, array('group_id' => $group_id));
