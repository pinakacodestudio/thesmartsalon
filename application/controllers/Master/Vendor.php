...
class Vendor extends CI_Controller
{
    ...
    public function index($id = 0)
    {
        ...
        if ($id != 0 && $id != "" && is_numeric($id)) {
            ...
            $query = "select * from " . TBL_VENDOR . " where isdelete=0 and id=" . $id . $cid;
            $params["vendor"] = $this->Queries->getSingleRecord($query);
        }
        $params["managerhide"] = 1;
        $this->load->view('Master/Vendor/index', $params);
    }
    public function save()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($id != 0 and $id != "") {
                ...
                $qry = "select vendor_name from " . TBL_VENDOR . " where mobile='" . $mobile . "' and id!=" . $id . $checkcid;
                ...
                if ($res != null) {
                    ...
                    return redirect('Master/Vendor/Index/' . $id);
                }
                ...
                $qry = "select vendor_name from " . TBL_VENDOR . " where mobile='" . $mobile . "' " . $checkcid;
                $res = $this->Queries->getSingleRecord($qry);
                if ($res != null) {
                    $this->session->set_flashdata('error_msg', 'Failed To Add Vendor Mobile No. Already Exists');
                    return redirect('Master/Vendor/Index/');
                }
