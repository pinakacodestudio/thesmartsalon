...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata['logged_in']['userid'] > 0 and is_numeric($this->session->userdata['logged_in']['userid'])) {
	...
	public function forgot_password()
	{
		...
		if ($this->config->item('identity', 'ion_auth') != 'user_email') {
		...
		if ($this->form_validation->run() === false) {
			...
			if ($forgotten) {
				$config = [
					'protocol' => 'smtp',
					'smtp_host' => 'ssl://sg2plcpnl0079.prod.sin2.secureserver.net',
					'smtp_port' => 465,
					'smtp_user' => 'support@thesmartsalon.com',
					'smtp_pass' => 'SupSmart@2019',
					'mailtype' => 'html'
				];
				$data = array(
				...
				$this->email->from('support@thesmartsalon.com');
				$this->email->to($forgotten['identity']);
				$this->email->subject("Reset password link for The Smart Salon");
